// Package policy is the authorization domain for kbridge: it maps a kubectl
// command to an AccessRequest, decides whether a subject may run it, and applies
// guardrails that block or gate dangerous commands. It has no transport or
// storage dependencies so the CLI can evaluate a policy file offline.
package policy
...
// Rule grants a set of verbs on resources within clusters/namespaces.
...
// Binding maps a subject (matched against the JWT email, may contain
...
// Policy is a parsed authorization document: role rules say what a subject may
// do, and guardrails carve dangerous commands back out of what they grant.
type Policy struct {
...
func (r Rule) allows(req AccessRequest) bool {
...
// Parse parses and validates a YAML policy document.
...
// validate checks that role names are unique, that all referenced roles
// (bindings and default) are defined, and that guardrails are well-formed.
...
// Engine holds the active policy and supports lock-free hot-swapping.
type Engine struct {
	current atomic.Pointer[Policy]
	path    string
}

// NewEngineFromFile loads a policy from path into a new engine.
...
// NewEngine returns an engine serving an already-parsed policy. The engine has
// no file backing it, so Reload and Watch are no-ops for it.
...
// NewEngineFromBytes parses a policy document and returns an engine serving it.
...
func (e *Engine) Allows(subject string, req AccessRequest) bool {
...
// kept.
func (e *Engine) Watch(stop <-chan struct{}) {
