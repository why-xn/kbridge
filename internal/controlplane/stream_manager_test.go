...
type fakeSender struct {
	mu   sync.Mutex
	sent []*agentpb.ControlPlaneStreamMessage
}

func (f *fakeSender) Send(m *agentpb.ControlPlaneStreamMessage) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func (f *fakeSender) sentMessages() []*agentpb.ControlPlaneStreamMessage {
	f.mu.Lock()
	defer f.mu.Unlock()
	out := make([]*agentpb.ControlPlaneStreamMessage, len(f.sent))
	copy(out, f.sent)
	return out
}
...
type recordingSender struct {
	mu   sync.Mutex
	msgs []*agentpb.ControlPlaneStreamMessage
}

func (r *recordingSender) Send(m *agentpb.ControlPlaneStreamMessage) error {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *recordingSender) last() *agentpb.ControlPlaneStreamMessage {
	r.mu.Lock()
	defer r.mu.Unlock()
	if len(r.msgs) == 0 {
		return nil
...
func TestSessionManager_PortForward(t *testing.T) {
	...
	// Agent->control plane routing lands on PfOutput.
