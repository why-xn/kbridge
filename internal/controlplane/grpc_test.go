...
type fakeOpenStream struct {
	...
	sent     chan *agentpb.ControlPlaneStreamMessage
	ctx      context.Context
}
...
func newFakeOpenStream(agentID string) *fakeOpenStream {
	f := &fakeOpenStream{
		...
		sent:     make(chan *agentpb.ControlPlaneStreamMessage, 4),
		ctx:      context.Background(),
	}
...
func (f *fakeOpenStream) Send(m *agentpb.ControlPlaneStreamMessage) error {
	f.sent <- m
	return nil
}
